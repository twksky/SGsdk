...
int DecodeAMRFileToWAVEFile(const char *pchAMRFileName, const char *pchWAVEFilename)
{
    ...
    if (fpamr == NULL) {
        printf(" if (fpamr == NULL)\n");
        return 0;
    }
    fseek(fpamr,0,SEEK_SET);
    fseek(fpamr,0,SEEK_END);
    
    long longBytes =ftell(fpamr);
    fseek(fpamr,0,SEEK_SET);
    
    // 检查amr文件头
    fread(magic, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
    printf("pchAMRFileName = %s longBytes = %ld \n", pchAMRFileName, longBytes);
    for (int i = 0; i < strlen(AMR_MAGIC_NUMBER); i++) {
        printf("######== amr[%d] = %c %c \n",i, magic[i], AMR_MAGIC_NUMBER[i]);
    }
    ...
    while (1) {
        ...
        if (!ReadAMRFrame(fpamr, amrFrame, stdFrameSize, stdFrameHeader)) {
            break;
        }
